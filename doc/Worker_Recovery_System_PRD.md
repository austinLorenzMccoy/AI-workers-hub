...
---
I studied doc/ (all 9 files, including Worker_Recovery_System_PRD.md and the WorkersHub Backend/Frontend PRDs) and the full backend/ + frontend/ implementation, then integrated the Worker Recovery System as a purely additive layer — nothing existing was renamed, dropped, or had its behavior changed.

Database — backend/supabase/migrations/split/part9_worker_recovery.sql (mirrored to 20260902000000_worker_recovery.sql, matching your existing part1–part8 / dated-migration convention):
- New referrer role; app_users gains contract_status, referral_code, hourly_rate_usd, paystack_recipient_code (all add column if not exists)
- New tables: worker_timesheets, pay_slips, payments, warning_events, worker_feedback, disputes, referrals, payout_requests, partner_contacts — each RLS-scoped following your get_my_role()/auth.uid() pattern
- 5 active warnings → auto contract termination (trigger); revoke drops it back → reinstated
- Referral payout gating ("all referred workers paid") enforced by a DB trigger, not just the UI
- Two summary views: worker_earnings_summary, referral_summary

Types/data/UI — types/index.ts & types/database.ts extended additively (new AppUser fields are optional so nothing existing breaks); lib/db.ts + lib/demo-data.ts got a full Worker Recovery section using your existing liveOrDemo fallback convention; /dashboard now branches by role into WorkerPortal / ReferrerPortal components (admin/manager/supervisor dashboard is untouched); new admin/manager pages /warnings, /feedback (admin-only, per PRD), /referrals, /partners, wired into nav, hasAccess, and middleware.

Payouts — lib/paystack.ts + POST /api/payouts/process: settles via real Paystack Transfer when PAYSTACK_SECRET_KEY + recipient code exist, otherwise degrades to "approve, settle manually, mark Paid" — never blocks an admin.

Verified non-breaking: npx tsc --noEmit (only pre-existing errors remain, confirmed by diffing against a stash of the base branch), npx vitest run — 182/182 frontend + 80/80 backend tests pass, and npm run build succeeds with all new routes compiling.

Docs: added a "Worker Recovery System" section to README.md, a wiring section to doc/backend_wiring_guide.md, and an "Implementation status" note to doc/implementation_plan.md.

Not built yet (flagged, not silently dropped): bulk Excel/CSV import for partner_contacts, Slack/email delivery for warning escalations, and collecting real bank details for Paystack recipients.

To apply live: run 20260902000000_worker_recovery.sql in Supabase after your existing migrations, then promote users to worker/referrer roles as usual.